...
class CComponent
{
	private $_e;
	private $_m;
    protected $salt = 'A#-F:Z_f@GeX*Xv_)2xcrOe5&2?Ql41P#I@_*__+)$9@!FL4%2%$4&DA^';
    public function getDBConnect()
    {
        if($this->getFConfig('db_driver') == 'dblib')
        {
            return $config = array(
                'connectionString'      => ''.$this->getFServer($_COOKIE['server'],'db_driver').':host='.$this->getFServer($_COOKIE['server'],'db_host').';dbname='.$this->getFServer($_COOKIE['server'],'db_name').'',
                'username'              => $this->getFServer($_COOKIE['server'],'db_user'),
                'password'              => $this->getFServer($_COOKIE['server'],'db_password'),
                'schemaCachingDuration' => 2000,
            );
        }
        elseif($this->getFConfig('db_driver') == 'sqlsrv')
        {
            return $config = array(
                'connectionString'      => ''.$this->getFServer($_COOKIE['server'],'db_driver').':server='.$this->getFServer($_COOKIE['server'],'db_host').';database='.$this->getFServer($_COOKIE['server'],'db_name').'',
                'username'              => $this->getFServer($_COOKIE['server'],'db_user'),
                'password'              => $this->getFServer($_COOKIE['server'],'db_password'),
                'charset'               => 'utf8',
                'schemaCachingDuration' => 2000,
    ...
    public function getSingleDBConnect($server='')
    {
        if($this->getFConfig('db_driver') == 'dblib')
        {
            return $config = array(
                'connectionString'      => ''.$this->getFServer($server,'db_driver').':host='.$this->getFServer($server,'db_host').';dbname='.$this->getFServer($server,'db_name').'',
                'username'              => $this->getFServer($server,'db_user'),
                'password'              => $this->getFServer($server,'db_password'),
                'schemaCachingDuration' => 2000,
                'class'                 => 'CDbConnection'
            );
        }
        ...
        {
            return $config = array(
                'connectionString'      => ''.$this->getFServer($server,'db_driver').':server='.$this->getFServer($server,'db_host').';database='.$this->getFServer($server,'db_name').'',
                'username'              => $this->getFServer($server,'db_user'),
                'password'              => $this->getFServer($server,'db_password'),
                'charset'               => 'utf8',
                'schemaCachingDuration' => 2000,
    ...
    public function getIpbDBConnect()
    {
        return array(
            'connectionString'      => 'mysql:host='.$this->getFConfig('ipbServer').';dbname='.$this->getFConfig('ipbDB').'',
            'emulatePrepare'        => true,
            'username'              => $this->getFConfig('ipbUser'),
            'password'              => $this->getFConfig('ipbPass'),
            'charset'               => 'utf8',
            'schemaCachingDuration' => 2000,
            'class'                 => 'CDbConnection'
        );
    ...
    public function getActionBlock($block,$path='')
    {
        if($path != '')
            echo $this->renderPartial("block/$path/$block");
        else
            echo $this->renderPartial('block/'.Yii::app()->controller->action->id.'/'.$block);
    }
    public function getActionHtml($html,$path='')
    {
        if($path != '')
            echo $this->renderPartial("html/$path/$html");
        else
            echo $this->renderPartial('html/'.Yii::app()->controller->action->id.'/'.$html);
    }
    public function getActionMenu()
    {
            echo $this->renderPartial('html/common/menu');
    }
    public function getTemplate($type = 'frontend')
    {
        if($type == 'frontend')
            return Yii::app()->request->baseUrl.'/skin/'.$type.'/'.$this->getFConfig('f_theme').'/';
        elseif($type == 'core')
            return Yii::app()->request->baseUrl.'/skin/'.$type.'/';
        elseif($type == 'backend')
            return Yii::app()->request->baseUrl.'/skin/'.$type.'/'.$this->getFConfig('b_theme').'/';
    }
    public function getFConfig($params='')
    {
        if(file_exists('protected/core_conf/'.$_COOKIE['server'].'_server_db_'.$_COOKIE['language'].'.ini'))
            $configs = parse_ini_file('protected/core_conf/'.$_COOKIE['server'].'_server_db_'.$_COOKIE['language'].'.ini');

        if($params != '')
            $entity = $configs[$params];
        else
            $entity = $configs;
        return $entity;
    }
    public function getFServer($server,$params='')
    {
        if(file_exists('protected/core_conf/'.$server.'_server_db_'.$_COOKIE['language'].'.ini'))
            $configs = parse_ini_file('protected/core_conf/'.$server.'_server_db_'.$_COOKIE['language'].'.ini');
    ...
    {
        $list = parse_ini_file('protected/core_conf/servers_url_list.ini');
        return $list;
    }
    public function getCPCharactersMenu()
    {
        $menuArray = array(
            'teleportation'=>$this->getFConfig('teleportationMItem'),
            'reset'=>$this->getFConfig('resetMItem'),
            'points'=>$this->getFConfig('pointsMItem'),
        );
    ...
    public function getCharDetailInfo($name)
    {
        if($this->getFConfig('showCharInfo'))
            return CHtml::link($name,Yii::app()->createUrl('char/inventory', array('id'=>$name)),array('onClick'=>'getModalWindow(this);return false;'));
        else
            return $name;
    }
    public function getMap($number)
    {
        $map	= array(
            '0'		=>	'Lorencia',
            '1'		=>	'Dungeon',
            ...
            '39'	=>	'Kanturu Tower',
            '40'	=>	'Silent Map',
            '41'	=>	'Reffuge of Balgass',
            '42'	=>	'Barracks of Balgass',
            '45'	=>	'Illusion Temple I',
            '46'	=>	'Illusion Temple II',
            '47'	=>	'Illusion Temple III',
            '48'	=>	'Illusion Temple IV',
            '49'	=>	'Illusion Temple V',
            '50'	=>	'Illusion Temple VI',
            '51'	=>	'Elbeland',
            '52'	=>	'Blood Castle 8',
            '53'	=>	'Chaos Castle VII',
    ...
    {
        if($this->getFConfig('versionBit') == '32')
            return 'FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF';
        elseif($this->getFConfig('versionBit') == '64')
            return 'FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF';
        elseif($this->getFConfig('versionBit') == '16')
            return 'FFFFFFFFFFFFFFFF';
    }
    public function getSQLString($string)
    {
        if (substr($string,0,2) == '0x')
            return $string	= substr($string,2);
        else
            return $string;
    }
    public function getGuildMarkString($string)
    {
    ...
    public function getOddEven($number)
    {
        if($number % 2 == 0)
            return 'even';
        else
    ...
    {
        $sizes = array(Yii::t(''.Yii::app()->request->cookies['language']->value.'', ' Bytes'), ' KB', ' MB', ' GB', ' TB', ' PB', ' EB', ' ZB', ' YB');
        if ($size == 0)
            return('n/a');
        else
            return (round($size/pow(1024, ($i = floor(log($size, 1024)))), 2) . $sizes[$i]);
    }
    public function rmSpace($string)
    {
